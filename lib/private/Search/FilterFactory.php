...
final class FilterFactory {
	public static function get(string $type, string|array $filter): IFilter {
		return match ($type) {
			...
			FilterDefinition::TYPE_GROUP => new Filter\GroupFilter($filter, \OC::$server->get(IGroupManager::class)),
			...
			FilterDefinition::TYPE_USER => new Filter\UserFilter($filter, \OC::$server->get(IUserManager::class)),
	...
	private static function getPerson(string $person): IFilter {
		...
		return match (count($parts)) {
			1 => self::get(FilterDefinition::TYPE_USER, $person),
			2 => self::get(... $parts),
		};
	}
}
